...
public class AbstractBeanContainerTest {
	...
	public void shouldGetBeanInstance() {
		...
		assertNotNull("Bean hasn't had InjectedBean injected into it", bean.getInjectedBean());
	}

	@Test
	public void shouldGetBeanByName() {
		assertNotNull(beanContainer);
		SimpleBean bean = (SimpleBean) beanContainer.getBeanByName("simpleBean");
	...
	public void shouldGetBeanByNameNoCast() {
		assertNotNull(beanContainer);
		SimpleBean bean = beanContainer.getBeanByName(SimpleBean.class, "simpleBean");
		assertNotNull("Could not locate bean instance by name", bean);
	}
